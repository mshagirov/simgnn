...
class VertexDynamics(Dataset):
    ...
    def edge_tensions(self, mg_dict=None, lambdaij_path=None, perim_path=None, p0_path=None, kp_path=None):
        ...
        - perim_path : location of a file with cell `Perimeters` numpy array w/ shape Frames x Cells x 1, or Frames x Cells.
...
class HaraAblation(VertexDynamics):
    '''
    For working with Y. Hara et al. amnioserosa ablation data. Hara ablation dataset *does not* have cell pressures, edge tensions,
    and T+0 velocities. Instead of edge tensions `HaraAblation` contains edge recoil results from laser ablation experiments.
    ...
    def __init__(self, root, window_size=5, transform=None, pre_transform=None, smoothing=False, sma_lag_time=None):
        ...
        - node2cell_index.npy : shape (2, Node-to-Cell edges), first row contains indices of the nodes/vertices, and second
                                row contains indices of the corresponding cell IDs (cell indices).
        - edge_recoils.npy : recoil values for all edges. All values are "np.nan" except for target edge(s), shape (Edges,).
                             E.g. `[nan,..., nan, 0.0238768, nan,...,nan]`.
        - frames.npy: (optional) frame numbers for the first dimension of array in `vtx_pos.npy`, shape (Frames,).

        Arg-s:
        - root : path to a root directory that contains folder with raw dataset(s) in a folder named "raw".
        Raw datasets should be placed into separate folders each containing outputs from a single ablation sample.
        E.g. root contains ["raw", "processed", ...], and in folder "raw/" we should have ["sample_i", "sample_j", ...]
        - window_size : number of past velocities to be used as node features ordered from `T-window_size` to `T-1`
                       `[x(T-window_size+1)-x(T-window_size),..., x(T-1)-x(T-2), x(T+0)-x(T-1)]`, where `x(T)` is
                       node position at time `T`. For more details see method `VertexDynamics.pos2nodeXY()`.
        - transform :  transform(s) for graph datasets (e.g. from torch_geometric.transforms ), used in parent class' loading method.
        ...
        Notes:
        - Velocities are approximated as 1st order differences of positions `x` (vtx_pos.npy) in subsequent frames:
          `velocity(T+0) = x(T+1) - x(T+0)` (*only* for features, as T+0 is absent in ablation data).
        - Use `pre_transform` for normalising and pre-processing dataset(s).
        - To be a valid sample in `HaraAblation` dataset, an ablation movie should have at least `window_size+1` frames,
          otherwise an example is ignored. `HaraAblation` uses the last `window_size+1` number of frames to produce
          `window_size` number of past velocities (`x`) and T+0 positions (`pos`).
        '''
        super(HaraAblation, self).__init__(root, window_size=window_size, transform=transform, pre_transform=pre_transform,
                                         smoothing=smoothing, sma_lag_time=sma_lag_time)
    ...
    def pos2nodeXY(self, pos_path=None):
        ...
        - pos_path: path to vertex positions file, e.g. 'vtx_pos.npy', containing an array with shape `(frames)xNx2`.
