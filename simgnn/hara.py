...
def get_new_cellxy_order(cellxy_tn, idpairs, NumOfPoints_t):
    ...
    newcellxy_tn[idpairs[idpairs[:, 0] != -111, 0], :] = cellxy_tn[idpairs[idpairs[:, 0] != -111, 1]]
    # check
...
def get_node_locations(labels, xbound, ybound):
    ...
    - nodenames : node names, a list of tuples. "nodenames" -- each node is named
                  by a tuple of cell labels that share a node.
...
def node_dict2graph(node_dict):
    ...
    v_newid = {v_i: el for el, v_i in enumerate([v_c for k, v_c in enumerate(v_names) if k in edges])}
    ...
    node2cell_index = np.concatenate([np.stack([np.full((len(v_c),), k, dtype=np.int64),
                                      np.array(v_c, dtype=np.int64)], axis=0)
...
def get_roi_cell_labels(img, v1_pos, v2_pos, s=1.0, min_roi_radius=15, trim_edges=True):
    ...
    L_roi = [el for el in np.unique(L_roi) if el != 0]
    ...
    roi_cellxy = Cpos[[el-1 for el in L_roi]]
...
def mask_to_position_graph(img, v_pos):
    '''
    Converts BW cell boundary mask (skeleton) to a graph:
                {"vertices":pixel positions array, "edges": pairs of indices in positions array}.
    ...
    pix_pos = np.stack([a[img != 0].ravel()
                        for a in np.meshgrid(np.arange(0, img.shape[1]),
                       np.arange(0, img.shape[0]))], axis=1)

    # Label of the position as a vertex index in "v0_pos_cortd"
    pix_labels = {tuple(vi_pos.tolist()): el for el, vi in enumerate(v_pos) for vi_pos in vi}
    ...
    for k in range(0, pix_pos.shape[0]):
        ...
        for el in range(k+1, pix_pos.shape[0]):
            if np.sum((pix_pos[el]-pix_pos[k])**2) < 2.01:
                # edge_index[i]: [src_pos_id,tgt_pos_id]
                pix_edge_index.append([k, el])
...
def prune_position_graph(pix_pos, pix_edge_index, pix_labels):
    ...
    for pos_i in range(pix_pos.shape[0]):
        ...
        new_edges = np.concatenate([
            np.array([[to_vi[k], to_vi[el]] for k in range(len(to_vi)) for el in range(k+1, len(to_vi))
                      ]).T.reshape(2, -1),
            np.array([[from_vi[k], from_vi[el]] for k in range(len(from_vi)) for el in range(k+1, len(from_vi))
                      ]).T.reshape(2, -1),
            np.array([[k, el] for k in to_vi for el in from_vi
                      ]).T.reshape(2, -1)], axis=1).astype(np.int_)
        # prune edges
        if new_edges.size < 1:
            continue
        pix_edge_index = np.concatenate([np.delete(pix_edge_index, np.logical_or(pix_edge_index[1] == pos_i,
                                                   pix_edge_index[0] == pos_i), axis=1), new_edges], axis=1)
...
def mask_to_graph(img, v1_pos, v2_pos, s=1.5, min_roi=15, trim_edges=True):
    ...
    roi_labels, roi_cellxy = get_roi_cell_labels(img, v1_pos, v2_pos, s=s, min_roi_radius=min_roi,
