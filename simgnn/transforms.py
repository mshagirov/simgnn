...
class PosEncNeRF(object):
    '''
    Positional encoding using Fourier features (NeRF scaling)
    ...
    def __init__(self, b=2.0, m=4):
        '''
        b : scale of the frequency (maximum frequency=b**(m-1); base of the exponent)
        m : number of frequencies (total number of features = 2 x m x n, where n = number of original features)
        '''
        self.b = b
        self.m = m
        self.nu = 2*math.pi*torch.exp(torch.arange(0, m)*math.log(b) ) # frequencies*2*pi
