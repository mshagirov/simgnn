'''
Uses wrong 'Lambda_ij': lambda mg,t: lmd_ij_ON**torch.cos(omega_ij*t+phase_ij)**2
'''
...
for sim_k in range(N_sim_runs):
    ...
    lmd_ij_ON = torch.normal(mean=2.1,std=1.0,size=( m.edges.size(0),1) ).clamp_(0,6).type(dtype).to(device)
